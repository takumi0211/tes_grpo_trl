...
# --- 出力テキストファイル名とログファイル名 ---
text_name = f"results/{model_name_name}_process.txt"
csv_log_name = f"results/{model_name_name}_llm_logs.csv"

# --- 書き出しグラフ名 ---
output_name = f"results/{model_name_name}_results"
...
with open(text_name, 'w', encoding='utf-8') as f:
    while not env.done:
        ...
        if is_control_time:
            ...
            while True:
                attempt += 1
                thought, action_index = get_llm_action(
                        current_time,
                        forecast_text,
                        current_storage,
                ...
                # 思考プロセスにエラーパターンが含まれているかチェック
                thought_text = (thought or "").strip()
                if not any(pattern in thought_text for pattern in LLM_ERROR_PATTERNS):
                    # エラーパターンが含まれていない場合は成功
                    break

                # 最大リトライ回数に達した場合
                if attempt >= MAX_LLM_RETRIES:
                    print(f"LLM returned error output after {attempt} attempts. Proceeding with fallback action: {action_index}.")
                    break

                # リトライを実行
                print(f"LLM returned error output (attempt {attempt}); retrying...")
